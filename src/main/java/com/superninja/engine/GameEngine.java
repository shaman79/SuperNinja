...
public class GameEngine implements TouchListener {
    ...
    private void updatePlaying(double deltaTime) {
        ...
        if (spawnTimer <= 0) {
            spawnObject();
            spawnTimer = currentSpawnInterval * (0.8 + Math.random() * 0.4);
        }
    ...
    private void spawnObject() {
        // 20% chance for burst spawn (2-4 pairs at once)
        int spawnCount = 1;
        if (Math.random() < 0.20) {
    ...
    private void checkSlicing() {
        for (GameObject obj : gameObjects) {
            ...
            if (player1Blade.isActive() && player1Blade.intersects(obj)) {
            ...
            if (player2Blade.isActive() && player2Blade.intersects(obj)) {
